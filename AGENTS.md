...
`kind` values: `ringbuf`, `hash`, `lru_hash_map`, `array`, `percpu_*`,
`lpm_trie`, `bloom_filter`, `data` (snake_case — kebab spellings are rejected
with "Unknown kind"). In `.bind()`, **every key except `kind` is
